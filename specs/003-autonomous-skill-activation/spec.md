...
- **FR-027A**: The repository MUST also contain an immutable release-candidate pilot selected independently of outcomes by taking, in source-corpus order, the first 8 clean automatic cases, first 3 irrelevant cases, first 2 explicit user-requested cases, and their corresponding first 2 paired no-intent cases. Both conditions MUST run the same 15 IDs in the declared order. This pilot is bounded release-candidate evidence, not a statistically definitive or universal harness claim. The 30-case 15/5/5/5 pair is an optional non-blocking extended benchmark, and the original 65 non-overlap cases remain an optional expanded benchmark.
...
- **FR-040**: Each manual evidence set MUST record the evaluation protocol, source corpus, frozen release-subset identity, frozen catalog, server policy, adapter presence and version, Codex product, model, reasoning setting, run date, environment profile, effective MCP/skill inventory condition, and per-case observable trace outcomes.
...
- **FR-043**: Live evaluation evidence MAY support release decisions but MUST remain outside required CI and MUST be reproducible without changing the frozen corpus or pilot or retroactively changing expected results. The validated server-only `0/7` evidence MUST remain preserved as an immutable baseline and MUST NOT be overwritten, relabeled as adapter evidence, or reinterpreted as successful activation. Release material MUST qualify the 15-case pair as pilot/release-candidate evidence and MUST NOT present its 80% metric as statistically definitive or generalize it to all prompts, models, clients, or harness versions.
...
- **SC-001**: In the version-recorded server-plus-adapter release-candidate pilot, at least 80% of the 8 selected relevant specialized prompts cause one spontaneous automatic-context search without mentioning SkillWire, MCP, the adapter, or operation names. This is a pilot target, not a statistically definitive estimate; passing supports only a qualified observation for the recorded configuration. No autonomous-activation claim may be made from server-only instructions unless separately measured evidence meets the same target.
...
- **SC-003**: No more than 10% of the 3 selected trivial or irrelevant adapter-pilot prompts cause any SkillWire operation.
- **SC-004**: In both evaluation conditions, both selected user-requested pairs enforce isolation: 100% exclude the opt-in skill without explicit intent, and 100% make it eligible when the corresponding explicit intent is present and all other eligibility rules pass.
...
- **SC-011**: An evaluator can reproduce the published pilot metric calculations from the versioned corpus, immutable 15-case subset, and privacy-safe per-case evidence using the documented denominators, with no access to private repositories or raw user conversations. The optional 30-case and 65-case benchmarks are reported separately.
...
- Runtime cost is part of evaluation design: the frozen 15-case stratified pilot is the bounded release-candidate cohort, the 30-case pair is optional extended evidence, and the full 65-case non-overlap pair is optional expanded evidence. Selection uses source-corpus order and was specified independently of outcomes; no case can be revised from observed success, failure, or latency.
