...
- [X] T024 [US2] Implement exact-version resolution, complete bundle verification, immutable-cache fallback re-verification, advisory availability/revocation behavior, and inert-text responses in `src/application/use-cases/load-skill.ts`, `src/catalog/version-controlled-provider.ts`, and `src/catalog/verified-revision-cache.ts`
- [X] T025 [US2] Wire `load_skill` through strict MCP schemas/adapters with full immutable provenance and no resource bodies in `src/transport/mcp/schemas.ts` and `src/transport/mcp/tool-adapters.ts`
...
- [X] T033 [US4] Add the tenant-first usage table and direct PostgreSQL repository-memory port/store with bounded list and idempotent exact-revision upsert in `migrations/002_repository_skill_usage.sql`, `src/application/ports/repository-memory-store.ts`, and `src/persistence/postgres/repository-memory-store.ts`
- [X] T034 [US4] Integrate optional repository context into load recording and direct-database search boost projection while preserving zero persistence for hash-free calls in `src/application/use-cases/load-skill.ts`, `src/application/use-cases/search-skills.ts`, and `src/domain/catalog/ranking.ts`
- [X] T035 [US4] Implement and wire `list_repo_memory` with strict account/hash scoping and constant empty-scope behavior in `src/application/use-cases/list-repo-memory.ts`, `src/transport/mcp/schemas.ts`, and `src/transport/mcp/tool-adapters.ts`
...
- [X] T048 CRITICAL: Replace catalog reads with descriptor-safe, no-follow, fixed-root-contained, pre-read-bounded access and add intermediate-symlink plus path-swap adversarial coverage in `src/catalog/catalog-loader.ts`, `src/domain/catalog/resource-path.ts`, and `tests/security/catalog/resource-safety.test.ts` per Constitution VI, FR-016, and FR-018 (contradicts)
- [X] T049 Filter zero-relevance catalog entries before ranking limits and add deterministic empty-result unit, contract, and end-to-end coverage in `src/domain/catalog/ranking.ts`, `tests/unit/domain/ranking.test.ts`, `tests/contract/mcp/search-skills.test.ts`, and `tests/e2e/search-skills.test.ts` per US1/AC4 and FR-008 (partial)
- [X] T050 Enforce the configured deadline across authentication, PostgreSQL access, MCP dispatch, and tool execution, then prove a genuinely slow in-flight operation fails within the bound in `src/observability/request-context.ts`, `src/transport/mcp/app.ts`, and `tests/security/transport/ssrf-and-execution-boundaries.test.ts` per T042 and security: HTTP, Logging, and Safe Failures (partial)
- [X] T051 Separate `catalog:verify` from publisher, writable-filesystem, migration, and PostgreSQL imports, and add an import-boundary assertion for the real verifier process in `src/catalog/admin-cli.ts`, `src/catalog/catalog-verifier.ts`, and `tests/contract/catalog-cli/catalog-verify.test.ts` per security invariant 9 and plan: Strictly Read-Only Verification (contradicts)
- [X] T052 Make the required `catalog:verify` CI path perform and report the fully paginated exact-commit GitHub release baseline for genesis and non-genesis releases instead of unconditionally rejecting non-genesis, while preserving fail-closed read-only behavior in `src/catalog/catalog-verifier.ts`, `src/catalog/github-release-baseline.ts`, `src/catalog/admin-cli.ts`, and `.github/workflows/ci.yml` per FR-037 and T012 (partial)
- [X] T053 Complete the real-command catalog CLI evidence matrix with every pre-rename fault point, existing release and later-release duplicate identity rejection, stale claim, concurrent publication, post-rename cleanup diagnostic, denied verifier write/database capabilities, workspace/database snapshots, and the frozen GitHub fixtures in `tests/contract/catalog-cli/catalog-publish.test.ts` and `tests/contract/catalog-cli/catalog-verify.test.ts` per FR-033 and T010 (partial)
- [X] T054 Gate readiness on live database/schema availability and require recovery cleanup before returning ready, with real PostgreSQL outage/recovery, concurrent cleanup, and restart tests in `src/lifecycle/audit-cleanup-scheduler.ts`, `src/lifecycle/readiness-state.ts`, `src/transport/mcp/app.ts`, `src/composition.ts`, `tests/integration/service/audit-cleanup-readiness.test.ts`, and `tests/integration/service/composition.test.ts` per FR-032 and SC-011 (partial)
- [X] T055 Extend the single monitored client-tree harness across corrupt catalog-cache failure, rate limiting, and repository-memory erasure while preserving byte, entry-type, and mode identity in `tests/e2e/no-client-write.test.ts` per SC-005, FR-033, T023, and T047 (partial)
- [X] T056 Implement distinct catalog-cold verification-per-operation and catalog-warm preloaded benchmark paths, deterministic disposable database fixture handling, complete required environment/version/digest/commit metadata, and full result-schema validation in `benchmarks/informational-benchmark.ts`, `benchmarks/result.schema.json`, and `compose.benchmark.yaml` per plan: Informational Measurement and T046 (partial)
- [X] T057 Remove full benchmark execution from required CI while retaining immutable input/result-schema validation and the manually invoked benchmark profile in `.github/workflows/ci.yml` per plan: Informational Measurement (contradicts)
