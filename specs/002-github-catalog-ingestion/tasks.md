...
- [X] T017 [US1] Create deterministic discovery pages plus rename, ETag/304, rate-limit, retry, cancellation, nested-layout, license conflict, dependency, second-commit, removal, incomplete-tree, and source-unavailability mutation fixtures before their implementations in `tests/fixtures/github-ingestion/discovery/` and `tests/fixtures/github-ingestion/mutations/`
- [X] T018 [P] [US1] Write failing discovery query/pagination/result/rate/byte budget, duplicate canonical repository, incomplete result, discovered-default-deny, and no-search-trigger tests in `tests/unit/ingestion/github-discovery-provider.test.ts` and `tests/integration/github-ingestion/discovery.test.ts`
...
- [X] T021 [P] [US5] Write failing strict `discover`, `verify`, `quarantine`, `curate`, and filtered `source:list` CLI contracts, authorization, idempotence, and no-write `catalog:verify` isolation tests in `tests/contract/source-cli/source-policy.test.ts` and `tests/contract/catalog-cli/catalog-verify.test.ts`
...
- [X] T023 [US5] Add verification reports/findings, append-only classification/curation history, current projections, and serialized external advisory-chain migration with immutable triggers in `migrations/006_external_policy_and_advisories.sql`
- [X] T024 [US1] Implement server-controlled GitHub search discovery, deterministic pagination, conditional metadata cache, shared budget ledger, and discovery evidence persistence in `src/ingestion/github/discovery-provider.ts`, `src/application/services/source-discovery-service.ts`, and `src/persistence/postgres/github-source-store.ts`
- [X] T025 [P] [US4] Implement persistent PostgreSQL lease acquisition/renewal/release, monotonic fencing checks, due-job claims, and shutdown-aware scheduled discovery/synchronization in `src/persistence/postgres/sync-lease-store.ts` and `src/lifecycle/github-sync-scheduler.ts`
- [X] T026 [US5] Implement bounded nested `SKILL.md` fallback, pinned SPDX license/notice/attribution validation, exact same-snapshot dependency resolution/evidence, stable findings, candidate policy, and legal classification transitions in `src/ingestion/parsing/nested-skill-layout.ts`, `src/domain/external-catalog/license-validator.ts`, `src/domain/external-catalog/dependency-resolver.ts`, and `src/domain/external-catalog/candidate-policy.ts`
- [X] T027 [US4] Extend synchronization with ETags, bounded retry/rate reset, whole-run cancellation, validated rename aliases, content/revision/resource deduplication, equality observations, atomic source-head moves, update/removal advisories, and fail-closed partial acquisition in `src/application/services/source-synchronization-service.ts` and `src/persistence/postgres/external-catalog-store.ts`
- [X] T028 [US5] Add administrator discovery/verification/quarantine/curation/list orchestration and safe structured results/reason codes while preserving immutable publication facts in `src/ingestion/admin-cli.ts` and `src/application/services/source-registration-service.ts`
- [X] T029 [US4] Make all Slice 2 state-machine, operational synchronization, persistence/restart, adversarial acquisition, redaction, cancellation, and Feature 001 regression suites pass via `tests/integration/github-ingestion/synchronization.test.ts` and `tests/security/github-ingestion/`
...
- [x] T033 [US3] Add an asynchronous cancellation-aware catalog port, wrap the unchanged first-party provider, implement direct verified PostgreSQL imported reads, and merge them deterministically in `src/application/ports/async-skill-catalog-provider.ts`, `src/catalog/static-provider-adapter.ts`, `src/persistence/postgres/external-catalog-provider.ts`, and `src/catalog/unified-provider.ts`
- [x] T034 [US3] Extend strict search schemas/use case/adapters with optional `invocationContext` defaulting to automatic, external-only discriminated previews, classification/advisory/invocation filtering before ranking, and memory boosts only after positive relevance in `src/transport/mcp/schemas.ts`, `src/application/use-cases/search-skills.ts`, and `src/transport/mcp/tool-adapters.ts`
- [x] T035 [US3] Extend exact load/resource use cases with canonical v2 re-verification, external provenance/license/invocation/dependencies, manifest-only load, single-resource bodies, unavailable verified reads, revoked denial, and transactional memory recording in `src/application/use-cases/load-skill.ts`, `src/application/use-cases/read-skill-resource.ts`, and `src/transport/mcp/tool-adapters.ts`
- [x] T036 [US3] Compose the unified provider and scheduler with strict live-ingestion configuration, PostgreSQL/readiness probes, clean shutdown, and no GitHub readiness dependency in `src/config.ts`, `src/composition.ts`, `src/main.ts`, and `src/lifecycle/readiness-state.ts`
- [x] T037 [US3] Implement imported evaluation loaders/threshold enforcement and make all 25 traceable acceptance outcomes, repository-memory cases, and three-call journeys pass in `src/evaluation/`, `tests/evaluation/github-import-search.test.ts`, and `tests/evaluation/github-import-journeys.test.ts`
- [x] T038 [P] [US3] Update production/container configuration, required recorded-fixture CI, source administration/operations/security documentation, and quickstart commands in `Dockerfile`, `compose.yaml`, `compose.test.yaml`, `.github/workflows/ci.yml`, `README.md`, `docs/operations.md`, and `specs/002-github-catalog-ingestion/quickstart.md`
- [x] T039 [P] [US3] Implement the optional token-gated fixed-commit live GitHub smoke command with no fixture rewrite or required-CI dependency in `scripts/smoke-github-live.ts`, `package.json`, and `.github/workflows/github-live-smoke.yml`
- [x] T040 [US3] Make the complete Slice 3 MCP contracts, 25-skill acceptance/evaluation, operational synchronization, repository-memory isolation, container restart/readiness/shutdown, and no-client-install journey pass through `tests/e2e/github-ingestion.test.ts` and `tests/evaluation/`
...
- [x] T043 CRITICAL Persist immutable original owner/repository coordinates with every external revision, stop reconstructing historical provenance from mutable source rows, and prove old and deduplicated exact revisions retain byte-identical provenance and load successfully after a GitHub rename per Constitution IV, FR-039, FR-043, FR-047, and SC-006 (contradicts)
- [x] T044 CRITICAL Harden GitHub object acquisition and import parsing against the complete specified hostile-input matrix, including encoded/control paths, structural collisions, strict response schemas, streamed aggregate limits, YAML structural limits, binary/control text, code-derived dependency false positives, and excluded hidden/fixture/deprecated trees, while retaining fixed-origin no-execution behavior per Constitution II, Constitution VI, FR-016, FR-019, FR-025, FR-030, T008, T009, T022, and T041 (contradicts)
- [x] T045 Implement durable PostgreSQL synchronization jobs using `github_sync_runs`, make administrator and scheduler entrypoints enqueue or idempotently reuse jobs, and add fenced claiming, terminal status, retry/backoff/rate-reset scheduling, stale-run recovery, and crash-safe discovery recovery per FR-008, the ingestion state machine, T025, and T028 (missing)
- [x] T046 Make `verify --candidate-id` revalidate and transition the exact acquired candidate without resolving a newer branch, enforce revocable administrator authority, and implement the contracted source/candidate filters, cursor pagination, bounded response shape, stable administrative quarantine reasons, and idempotent CLI behavior per FR-034 through FR-037, the source administration contract, T021, and T028 (contradicts)
- [x] T047 Enforce configurable absolute operation deadlines and cancellation across CLI, scheduler, GitHub backoff/body reads, leases, and PostgreSQL publication; recheck lease ownership and expiry immediately before commit; and implement the documented query, request, page, tree, candidate, resource, dependency, byte, retry, concurrency, and wall-time budgets without retrying before required GitHub rate-limit delays per FR-015, FR-055, the security model, the operational synchronization plan, T020, T022, T027, and T036 (partial)
- [x] T048 Preserve complete immutable legal provenance per external revision, including license and NOTICE evidence paths, blob and SHA-256 hashes, retained text and attribution, supported SPDX detection, and repository/manifest/skill-level precedence and conflict quarantine per FR-027 through FR-029, SC-002, SC-011, the data model, and T026 (partial)
- [x] T049 Convert deterministic manifest, declaration, schema, duplicate-identity, dependency, and policy failures into atomic candidate findings with stable quarantine reason codes and one traceable result for every affected skill while publishing eligible siblings without partial or unreported outcomes per FR-033, FR-037, US2/AC4, T019, T026, and T027 (contradicts)
- [x] T050 Make imported public skill identifiers collision-safe for maximum-length and normalization-colliding names by preserving a source/path-derived discriminator within the identifier limit, and add deterministic cross-source and same-source collision tests per FR-044, SC-005, and T035 (partial)
- [x] T051 Complete upstream-removal handling with authenticated uncached observations, canonical numeric-identity and newly discovered alias checks, successful-observation counter resets, transactional confirmation, and a snapshot advisory-chain head computed after all advisories created by that publication per FR-047, the ingestion state machine, and T027 (partial)
- [x] T052 Extend required regression gates for T043 through T051, including historical load after rename, durable job crash recovery, final lease-expiry races, exact candidate reverification, admin authorization/idempotence, deterministic quarantine, public-ID collision, and imported PostgreSQL outcome-ranking journeys; remove live GitHub access from required CI; and make the manual token-gated smoke import and validate the fixed 25-skill/21-resource fixture in disposable PostgreSQL without rewriting fixtures per the testing strategy, T039, T041, and T042 (partial)

## Phase 8: Convergence

- [x] T053 HIGH Persist each external snapshot's `advisory_chain_head_sha256` as the final chain head after all availability advisories created by that publication, while retaining the verified pre-sync head comparison and fenced transaction, and add a regression proving the stored snapshot anchor equals the resulting chain head per T051, FR-047, and plan: data model (partial)
- [x] T054 HIGH Add a real PostgreSQL-backed imported-skill outcome-ranking journey that records and replaces outcomes through MCP, proves account/repository isolation, and verifies the bounded useful/neutral/unsuccessful ranking behavior without using `FakeRepositoryMemoryStore` per T052, FR-053, and Constitution VII (partial)
- [x] T055 HIGH Pass the live-GitHub network blocker into every required CI test execution, including the Compose test container, and add a delivery-integrity assertion that an unexpected `https://api.github.com` request fails closed there while the credentialed live smoke remains manual per T052 and the offline CI testing strategy (partial)
