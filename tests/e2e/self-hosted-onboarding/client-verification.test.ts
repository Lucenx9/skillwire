...
describe("fresh-process deterministic client verification", () => {
  ...
  it("runs exact bridge registration, six tools, search/load/resource, provenance, advisory, and post-inventory", async () => {
    ...
    const credentialReference = await store.store("codex", token, true);
    await writeFile(
      ...
      `${JSON.stringify({
        ...
        clients: [{ client: "codex", credentialReference }],
      })}\n`,
      { mode: 0o600 },
    );
